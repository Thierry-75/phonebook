...
public class Contact {
	...
	public String nombreContact(List<Contact> list) {
		...
		if (count == (int) count) {
			try {
				...
				for (int i = 0; i < count; i++) { // boucle pour la saisie des noms et des nombres (names et numbers)
					...
					String name = scanner.nextLine().toLowerCase(); // toLowerCase transforme toutes les lettres en
																	// minuscule
					if (!name.isEmpty() && name.matches("^[a-zA-Z]*$")) { // controle pas vide et conforme aux lettres
																			// de l'alphabet
						System.out.print("Saisir son numéro : ");
						String number = scanner.nextLine();
						if (!number.isEmpty() && number.matches("^[0-9]*$")) { // controle si 10 chiffres telephone
							list.add(ajouterContact(name, number)); // <---- liste d'objets Contact ajout de contact.nom
																	// - contact.numero
						} else {
	...
	public String rechercherContact(List<Contact> list) {
